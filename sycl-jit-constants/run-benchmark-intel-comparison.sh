...
# Runs both JIT and AOT benchmarks and compares results
# This demonstrates that specialization constants ONLY work with JIT compilation

echo "================================================================"
echo "Intel SYCL: JIT vs AOT Compilation Comparison"
echo "================================================================"
echo ""
echo "This script compares:"
echo "  JIT Mode:  -fsycl (SPIR-V + runtime JIT)"
echo "  AOT Mode:  -fsycl -fsycl-targets=spir64 (ahead-of-time native)"
echo ""
...
# Run JIT benchmark
echo "Running JIT mode benchmark..."
echo ""
./run-benchmark-intel-jit.sh
JIT_EXIT_CODE=$?
...
# Run AOT benchmark
echo "Running AOT mode benchmark..."
echo ""
./run-benchmark-intel-aot.sh
AOT_EXIT_CODE=$?
...
JIT_SPEEDUP=$(grep "Actual speedup:" build-benchmark-intel-jit/benchmark_results_jit_*.txt 2>/dev/null | tail -1 | awk '{print $3}' || echo "N/A")
AOT_SPEEDUP=$(grep "Actual speedup:" build-benchmark-intel-aot/benchmark_results_aot_*.txt 2>/dev/null | tail -1 | awk '{print $3}' || echo "N/A")
