...
# Intel JIT mode benchmark - device code JIT compiled at runtime
# This is the mode where specialization constants SHOULD work

echo "=============================================="
echo "SYCL Constant Folding Benchmark Suite"
echo "Intel oneAPI - JIT Mode"
echo "=============================================="
echo ""
...
if [ -f "$ONEAPI_ROOT/setvars.sh" ]; then
    ...
    set +e
    # Do not silence errors; capture exit code instead
...
BUILD_DIR=build-benchmark-intel-jit
mkdir -p $BUILD_DIR
cd $BUILD_DIR
...
echo "=============================================="
echo ""
echo "Compilation mode: JIT (SPIR-V + runtime compilation)"
echo "  -fsycl (no -fsycl-targets)"
...
SYCL Constant Folding Benchmark Results (Intel JIT Mode)
=========================================================
Date: $(date)
Compiler: $(icpx --version | head -1)
Mode: JIT (SPIR-V with runtime JIT compilation)
...
Expected: In JIT mode, specialization constants SHOULD enable optimization
